...
public:
  ...
  void loadRefs(Ref<VectorX> inputVec);
  void returnObservedScores(Ref<MatrixX> ObservedScores);

  // Results
  MatrixX internalObservedScores;
  VectorX internalBestScoreEachTimestep;

  // Reference objective values
  VectorX internalRefScores;
