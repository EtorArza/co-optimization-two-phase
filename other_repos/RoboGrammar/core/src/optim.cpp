...
MPPIOptimizer::MPPIOptimizer(
  ...
  this->internalObservedScores = MatrixX::Zero(sample_count_, horizon_ * interval_);
  this->internalRefScores = VectorX::Zero(horizon_ * interval_);
...
void MPPIOptimizer::setSampleCount(int sample_count) {
  if (sample_count <= sample_count_) {
    ...
    internalObservedScores.resize(sample_count_, horizon_);
  } else {
...
void MPPIOptimizer::loadRefs(Ref<VectorX> inputVec)
{
  for (int i = 0; i < horizon_; i++)
  {
    this->internalRefScores[i] = inputVec[i];
  }  
}


void MPPIOptimizer::returnObservedScores(Ref<MatrixX> ObservedScores)
{
  for (int sample = 0; sample < sample_count_; sample++)
  {
    for (int i = 0; i < horizon_; i++)
    {
      // std::cout << internalObservedScores(sample,i) << std::endl;
      ObservedScores(sample,i) = internalObservedScores(sample,i);
    }
  }
}
...
// void MPPIOptimizer::loadVec(Ref<VectorX> inputVec, Ref<VectorX> resVec)
// {
//   this->internalObservedScores = MatrixX::Zero(3, 4);
//   this->internalRefObjectives = VectorX::Zero(3);
//   std::cout << horizon_ << "," << interval_ << std::endl;
//   resVec[0] = -99.0;
// }
...
Scalar MPPIOptimizer::runSimulation(unsigned int sample_seed, int sample_idx) {
  ...
  for (int j = 0; j < horizon_; ++j) {
    ...
    internalObservedScores(sample_idx,j) = sim_return;
