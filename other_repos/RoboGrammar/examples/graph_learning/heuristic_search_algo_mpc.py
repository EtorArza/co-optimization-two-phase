...
def predict_batch(V, states):
    ...
    max_nodes = 0
    for state in states:
        adj_matrix, features, _ = preprocessor.preprocess(state) # This is the slow line
...
def search_algo(args, resfilepath, mode):
    ...
    print("Begin torch set threads... ", end="")
    ...
    if not args.test:
        ...
        for epoch in range(args.num_iterations):
            ...
            for _ in range(args.num_eval):
                _, rew = env.get_reward(selected_design, no)
                if rew > reward:
            ...
            for _ in range(args.opt_iter):
                ...
                train_adj_matrix_torch = torch.tensor(np.array(train_adj_matrix))
                train_features_torch = torch.tensor(np.array(train_features))
                train_masks_torch = torch.tensor(np.array(train_masks))
                train_reward_torch = torch.tensor(np.array(train_reward))
